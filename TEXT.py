...
def prepare_keyword_6x6(keyword: str) -> str:
    ...
    # Prepare keyword for 6x6 Playfair: Uppercase, Keep only A-Z and 0-9, Remove duplicates (keep first occurrence)
...
def clean_plaintext_6x6(text: str) -> str:
    ...
    # Clean plaintext: Uppercase, Keep only A-Z and 0-9 (remove spaces and punctuation)
...
def make_digraphs(text: str, padding_char: str = 'X'):
    ...
    # Split text into Playfair digraphs: If pair letters are the same, insert padding_char after first, If odd length, pad last with padding_char
...
def remove_padding_x(decrypted: str) -> str:
    ...
    # Heuristic: If X is between two identical letters (A X A), treat as padding and remove it, If X is the very last character, treat as padding and remove it, Keep other X's (they might be real).
...
def restore_spacing(original_text: str, decrypted_text: str) -> str:
    
    # Restore original word spacing using the word lengths from the original plaintext.
    
    words = original_text.upper().split()
    index = 0
    spaced = []

    for word in words:
        length = len(clean_plaintext_6x6(word))
        spaced.append(decrypted_text[index:index+length])
        index += length
...
def playfair_q2():
    ...
    plaintext = "Let us meet at 1234 Drive Street after lunch"
    print("\n[+] Original plaintext:")
    print(plaintext)
    ...
    # Restore spacing for display
    restored = restore_spacing(plaintext, cleaned_decrypted)

    print("\n[+] Decrypted text with restored spacing:")
    print(restored)
